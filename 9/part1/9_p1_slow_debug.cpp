        ...
        for (int ci = 0; ci < m_w; ++ci)
        {
            if (m[ri][ci] == '#')
            {
                ...
                for (int rq = ri; rq < m_h; ++rq)
                {
                    ...
                    int far_sharp_x = -1;
                    for (int ck = ci; ck >= 0; --ck)
                    {
                        if (m[rq][ck] == '#')
                            far_sharp_x = ck;
                    }
                    if (far_sharp_x == ci && rq == ri) // we don't count ourselves
                        far_sharp_x = -1;
                    // far_sharp_x contains the coordinate of the farthest '#'
                    // Let's count the area
                    if (far_sharp_x != -1) // sharp on the left was found
                    {
                        u_ll area = (ci - far_sharp_x + 1) * (rq - ri + 1);
                ...
                for (int rq = ri; rq < m_h; ++rq)
                {
                    ...
                    if (far_sharp_x != -1) // sharp on the left was found
                    {
                        u_ll area = (far_sharp_x - ci + 1) * (rq - ri + 1);
                        Tile tmp_tile1(ci, ri);
                        Tile tmp_tile2(far_sharp_x, rq);
                        Area *tmp_area = new Area(area, &tmp_tile1, &tmp_tile2);
                        areas.push_back(tmp_area);
